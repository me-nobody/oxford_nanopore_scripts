...
echo $PWD
detect_path="/rds/projects/b/broderra-mrc-alt-telomere/Anu/pilot_data/Nanopore_271123/dnascent"
container_path="/rds/projects/b/broderra-mrc-alt-telomere/Anu/containers" 

echo $(wc -l $detect_path/posctl_271123_CPU.detect)

apptainer run  $container_path/DNAscent.sif forkSense \
              --detect $detect_path/posctl_271123_CPU.detect  \
              --output "$detect_path/posctl_CPU_forksense.bed" \
              --order "EdU,BrdU" \
              --markAnalogues \
              --markTerminations \
              --markForks \
